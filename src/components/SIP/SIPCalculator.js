  ...
  return (
    <div className="mx-auto w-full max-w-[1400px] md:rounded-2xl rounded-2xl md:p-8 p-4 bg-white dark:bg-gray-900 shadow-lg ring-1 ring-gray-100 dark:ring-gray-800 transition-colors duration-300 md:w-[90%] sm:w-full">
      ...
      <div className="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <ToggleSwitch active={mode} onToggle={setMode} />
      </div>
      ...
      {/* 📈 Comparison Chart */}
      <div className="mt-10 rounded-2xl border border-gray-200 dark:border-gray-700 p-5 shadow-sm bg-white dark:bg-gray-800 transition-colors duration-300">
        <div className="mb-4 flex items-center justify-between">
          <h2 className="text-lg font-semibold text-gray-900 dark:text-gray-100">
            SIP vs Lumpsum — Growth Over Time
          </h2>
          <div className="text-sm text-gray-500 dark:text-gray-400">
            Comparing with same rate ({data.annualRate}% p.a.)
          </div>
        </div>
        ...
        <div className="mt-4 grid gap-3 sm:grid-cols-3">
          <InfoBox
            label="Monthly SIP"
            value={pretty(data.monthlyInvestment)}
            color="emerald"
          />
          <InfoBox
            label="Lumpsum"
            value={pretty(data.lumpsumInvestment)}
            color="sky"
          />
          <InfoBox
            label="Tenure"
            value={`${data.years} Years`}
            color="indigo"
          />
        </div>
      </div>
